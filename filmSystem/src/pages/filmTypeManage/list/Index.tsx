...
import {IFilmType, resourcesTypeDelete, resourcesTypeList} from "../../../api/filmType";


const UserList = () => {
    const columns: ColumnsType<IFilmType> = [
        {
            title: '类型名称',
            dataIndex: 'name',
            key: 'name'
    ...
    const [data, setData] = useState<IFilmType[]>()
    ...
    useEffect(() => {
        (async () => {
            const {data: {list}} = await resourcesTypeList({})
            setData(list)
        })()
    }, [a]);
    return <>
